...
@curry
def discount_fn(df: pd.DataFrame, discount_col: str, max_discount: float = 0.5):
    n = df.shape[0]
...
@curry
def payment_fn(df: pd.DataFrame, payment_col: str, initil_debt_col: str, debt_col: str, intr_col: str, fin_ed_col: str):
...
class Environment(object):
    ...
    @staticmethod
    def get_new_customers(date: datetime.datetime, n: int):
        return (pd.DataFrame({
    ...
    @staticmethod
    def update(old: pd.DataFrame, date: datetime.datetime):
        return (old
                .assign(date=date)
                ...
                .pipe(discount_fn(discount_col="discount"))
    ...
    def next_day(self, intervention: Dict[str, Callable[[pd.DataFrame], pd.Series]]):
